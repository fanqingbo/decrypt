...
object PageRankBigData {
  ...
  def main(args: Array[String]) {
    //屏蔽日志
        ...
        //读入数据文件
        val articles: RDD[String] = sc.textFile("/user/fanqingbo/graphx-wiki-vertices.txt")
        val links: RDD[String] = sc.textFile("/user/fanqingbo/graphx-wiki-edges.txt")
        //装载顶点和边
    ...
    //0.001为前后两次收敛的误差阈值，小于这个阈值时则结束计算，越小精度越到
